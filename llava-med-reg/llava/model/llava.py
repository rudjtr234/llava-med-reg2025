...
#                assert  num_new_tokens == 2
                ...
                im_start_id, im_end_id = tokenizer.convert_tokens_to_ids([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN])

                if embed_tokens_weight.shape[0] == 2:
                    input_embeddings[im_start_id] = embed_tokens_weight[0]
                    input_embeddings[im_end_id] = embed_tokens_weight[1]
                else:
                    raise ValueError(f"Expected 2 delta tokens but got shape {embed_tokens_weight.shape}")
